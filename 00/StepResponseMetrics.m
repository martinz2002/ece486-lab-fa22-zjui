...
t_10index = find(  y > 0.1*ssVal, 1, 'first');
% I've done the 10% index, you do the 90%:
t_90index  = find( y <= 0.9*ssVal, 1, "last");
...
while (abs(y(x) - ssVal) / ssVal) < 0.05    %PLACE YOUR CONDITIONS HERE
    x = x-1;
end
...
% document tss
text(t(tssIndex),y(tssIndex),'\leftarrow SteadyState','HorizontalAlignment','left')
line([t(tssIndex);t(tssIndex)],[0,y(tssIndex)],'Color','k','LineWidth',0.5,'LineStyle',':')
